...
def read_screen_names(filename):
    ...
    try:
        with open(filename, 'r') as file_object:
            screen_names = list(file_object)
    except Exception as e:
        print("Not able to read the file as " + str(e))
        raise
    # To remove "\n" character
...
def get_users(twitter, screen_names):
    ...
    [6253282, 783214]
    """
    screen_names_comma_separated = ','.join(map(str, screen_names))
    request = robust_request(twitter, 'users/lookup', {'screen_name': screen_names_comma_separated})
...
def get_friends(twitter, screen_name):
    ...
    return sorted(json.loads(request.text)["ids"][:5000])
...
def print_num_friends(users):
    ...
    for user in sorted(users, key=lambda x: x['screen_name']):
        print(str(user["screen_name"]) + " " + str(len(user["friends"])))
...
def friend_overlap(users):
    ...
    for i in range(len(users) - 1):
        user1_screen_name = users[i]['screen_name']
        user1_friends = users[i]['friends']
        for j in range(i + 1, len(users)):
            user2_screen_name = users[j]["screen_name"]
            user2_friends = users[j]["friends"]
            tuple = (user1_screen_name, user2_screen_name, len(list(set(user1_friends) & set(user2_friends))))
            list_of_tuples.append(tuple)
    # Sort by user1, user2 in ascending alphabetical order
    list_of_tuples_sorted = sorted(list_of_tuples, key=lambda tup: (tup[0], tup[1]))
    # Sort by N (number of accounts that both user1 and user2 follow) in descending order
    return sorted(list_of_tuples_sorted, key=lambda tup: tup[2], reverse=True)
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    hillary_clinton_friends = []
    donald_trump_friends = []
    for user in users:
        if user['screen_name'] == 'HillaryClinton':
            hillary_clinton_friends = user['friends']
        if user['screen_name'] == 'realDonaldTrump':
            donald_trump_friends = user['friends']
    followed_by_hillary_and_donald_list = list(set(hillary_clinton_friends) & set(donald_trump_friends))
    request = robust_request(twitter, 'users/lookup', {'user_id': ','.join(map(str, followed_by_hillary_and_donald_list))})
    return json.loads(request.text)[0]['screen_name']
...
def draw_network(graph, users, filename):
    ...
    label_nodes = [user['screen_name'] for user in users]
    labels = {}

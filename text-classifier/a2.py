...
def token_pair_features(tokens, feats, k=3):
    ...
    """
    windows = []
    num_windows = tokens.size - (k - 1)
    for window in range(num_windows):
        token = tokens[window:window + k]
        windows.append(token.tolist())

    for window in windows:
        for comb in list(combinations(window, 2)):
            token='token_pair=' + comb[0] + '__' + comb[1]
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for doc in tokens_list:
        features.append(featurize(doc, feature_fns))

    if vocab == None:
        tokens = []
        for feature in features:
            tokens.extend([a[0] for a in feature if a[1] > 0])
        #tokens_dict = dict(tokens)
        tokens_dict = defaultdict(lambda: 0)
        for token in tokens:
            tokens_dict[token] += 1
        filter_vocab = sorted([a for a in tokens_dict if tokens_dict[a] >= min_freq])
        vocab = dict()
        col = 0
        for feat in filter_vocab:
    ...
    for punct_val in punct_vals:
        for min_freq in min_freqs:
            for Ln in range(1, len(feature_fns) + 1):
                for fun_set in combinations(feature_fns, Ln):
                    docs_list = [tokenize(doc, punct_val) for doc in docs]
                    X, vocab = vectorize(docs_list, fun_set, min_freq)
                    if vocab:
...
def fit_best_classifier(docs, labels, best_result):
    ...
    X, vocab = vectorize(docs_list, features, min_freq)
    clf = LogisticRegression()
    clf.fit(X, labels)
    return clf, vocab
    pass
...
def top_coefs(clf, label, n, vocab):
    ...
    Returns:
      ...
      given class label.
    """
    vocab_elements = np.array([k for k, v in sorted(vocab.items(), key=lambda tup: tup[1])])
    coef = clf.coef_[0]

    if label == 1:
        top_coef_index = np.argsort(coef)[::-1][:n]
    else:
        top_coef_index = np.argsort(coef)[:n]

    top_coef_terms = vocab_elements[top_coef_index]
    top_coef = abs(coef[top_coef_index])
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    predicted_probabilties = clf.predict_proba(X_test)
    misclassified = []
    for i in range(len(predicted_values)):
        if predicted_values[i] != test_labels[i]:
            entry = dict()
            entry['truth'] = test_labels[i]
            entry['predicted'] = predicted_values[i]
            entry['proba'] = predicted_probabilties[i][entry['predicted']]
            entry['document'] = test_docs[i]
            misclassified.append(entry)
    misclassified = sorted(misclassified, key=lambda x: -x['proba'])[:n]
    for docEntry in misclassified:

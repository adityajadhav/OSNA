...
def bfs(graph, root, max_depth):
    ...
    while len(q) > 0:
        ...
        for adj_node in graph.neighbors(current_node):
            if node2distances[current_node] + 1 > max_depth:
                q.clear()
            else:
                if node2distances[current_node] + 1 == node2distances[adj_node]:
                    node2parents[adj_node].append(current_node)
                    node2num_paths[adj_node] = len(node2parents[adj_node])
                if adj_node not in seen:
                    seen.add(adj_node)
                    node2parents[adj_node].append(current_node)
                    node2distances[adj_node] = node2distances[current_node] + 1
                    node2num_paths[adj_node] = len(node2parents[adj_node])
                    q.append(adj_node)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for node in node2distances:
        node_credits[node] = 1
        if node2distances[node] in node_level:
            node_level[node2distances[node]].append(node)
        else:
            lnodes = list()
            lnodes.append(node)
            node_level[node2distances[node]] = lnodes
    node_level_sorted = sorted(node_level.keys(), reverse=True)

    for level in node_level_sorted:
        nodes = node_level[level]
        for node in nodes:
            parents = node2parents[node]
            for parent in parents:
                weight = float(node_credits[node] / len(parents))
                node_credits[parent] += weight
                t = (node, parent)
                result[tuple(sorted(t))] = weight
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        ...
        for r in result:
            if r in approximate_betweenness:
                approximate_betweenness[r] += result[r]
            else:
                approximate_betweenness[r] = result[r]

    for ab in approximate_betweenness:
        approximate_betweenness[ab] = (approximate_betweenness[ab] / 2)

    return approximate_betweenness
    pass
...
def partition_girvan_newman(graph, max_depth):
    ...
    approx_betweenness = approximate_betweenness(graph_clone, max_depth)
    sorted_approx_betweenness = sorted(approx_betweenness.items(), key=lambda a: (-a[1], a[0]))

    components = [nx.connected_component_subgraphs(graph_clone)]

    while len(components) == 1:
        edge = sorted_approx_betweenness.pop(0)[0]
        graph_clone.remove_edge(*edge)
        components = list(sorted(nx.connected_component_subgraphs(graph_clone), key=len, reverse=True))
    return components
    pass
...
def cut(S, T, graph):
    ...
    for node_s in S:
        for node_t in T:
            if graph.has_edge(node_s, node_t):
                count += 1
    return count
    pass
...
def norm_cut(S, T, graph):
    ...
    norm_c = float((cut_c / v_s) + ((cut_c / v_t)))
    return norm_c
    pass
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        norm_c = norm_cut(components[0], components[1], graph)
        score_max_depths.append((depth, norm_c))
...
def make_training_graph(graph, test_node, n):
    ...
    ['F', 'G']
    """
    clone_graph = graph.copy()
    neighbors = sorted(clone_graph.neighbors(test_node))[:n]
    for node in neighbors:
        clone_graph.remove_edge(test_node, node)
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        if n not in graph.neighbors(node) and n != node:
            j_score = len(set(graph.neighbors(node)) & set(graph.neighbors(n))) / len(
                set(graph.neighbors(node)) | set(graph.neighbors(n)))
            result.append(((node, n), j_score))
...
def path_score(graph, root, k, beta):
    ...
    for node in graph.nodes():
        if node not in graph.neighbors(root) and node != root:
            i = node2distances[node]
            n = node2num_paths[node]
            p_score = beta ** i * n
            result.append(((root, node), p_score))

    result = sorted(result, key=lambda a: (-a[1], a[0][1]))[:k]
    return result
    pass
...
def evaluate(predicted_edges, graph):
    ...
    for edge in predicted_edges:
        if graph.has_edge(*edge):
            c += 1
    f_exist = float(c / len(predicted_edges))
    return f_exist
    pass
